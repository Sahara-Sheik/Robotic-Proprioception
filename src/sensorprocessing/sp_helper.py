...
def load_picturefile_to_tensor(picture_file, transform, device):
    """Loads an image from a file using PIL and returns it as a single image batch and the image in a displayable format.
    """
    image = Image.open(picture_file)
    return load_image_to_batch(image, transform, device)

def load_capture_to_tensor(image_from_camera, transform, device):
